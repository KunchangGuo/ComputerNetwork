#ifndef SERVER_PACKETMANAGER_H
#define SERVER_PACKETMANAGER_H
...
class PacketManager {
public:
#define TOTAL_LEN(packet) (HEADER_SIZE + packet->header.length)
#define DATA_LEN(packet) (packet->header.length)
#define FILE_INFO_LEN(file_info) (sizeof(u16) + sizeof(u32) + sizeof(u16) + file_info->file_name_len)
...
#define MAKE_FILE_INFO(MSS, file_size, file_name) PacketManager::make_file_info(MSS, file_size, file_name)
    ...
    static File_Info* make_file_info(u16 MSS, u32 file_size, const string& file_name);  // make a file info packet
};


#endif //SERVER_PACKETMANAGER_H

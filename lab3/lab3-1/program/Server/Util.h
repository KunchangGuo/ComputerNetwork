...
#define TIME_SPAN(time_point) Util::time_span(time_point)
#define TIME_OUT(start) (TIME_SPAN(start) >= time_out)
...
enum TimeStampType {
    ...
    static unsigned time_span(chrono::high_resolution_clock::time_point start); // get time span
...
#define DUMP_DATA(file, data, data_len) Util::dump_data(file, data, data_len)
    ...
    static unsigned short read_data(fstream* file, unsigned char* buff, unsigned short max_segment_size);  // return the number of bytes read
    static bool dump_data(fstream* file, unsigned char* data, unsigned short data_len);
};

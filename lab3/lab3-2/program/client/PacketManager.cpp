...
File_Info* PacketManager::make_file_info(u32 primary_seq_num, u16 MSS, u32 file_size, const string& file_name){
    File_Info* file_info = new File_Info();
    file_info->primary_seq_num = primary_seq_num;
    file_info->MSS = MSS;
...
u16 PacketManager::get_data_length(UDP_Packet *packet) {
    return packet->header.length;
}

u16 PacketManager::get_total_length(UDP_Packet *packet) {
    return packet->header.length + HEADER_SIZE;
}
